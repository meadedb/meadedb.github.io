...
\end{center}}
\end{center}
\caption{Locus %of the point $\overrightarrow{OI}+\overrightarrow{IF}+\overrightarrow{IG}$
%               (a) $t=0$ and (b) $t=\pi$.
 %              Also, the entire locus of  $\overrightarrow{OI}+r\overrightarrow{IF}+s\overrightarrow{IJ}$
...
\begin{align*}
&
\includegraphics[natheight=0.818700in,natwidth=6.659500in,height=0.8329in,width=6.5628in]{PJH75I1M.pdf}
\\
&
\includegraphics[natheight=0.712200in,natwidth=6.659500in,height=0.7273in,width=6.5628in] {PJH75I1N.pdf}
\end{align*}

For comparison, Geometry Expressions \cite{GE} reports the corresponding
parametric equations for the locus of the point $J$ as
\[
\begin{array}
[c]{l}%
X=-(1-s)\left\vert a\right\vert -\\
\dfrac{2a^{2}s\sin(t)\cos(t)\left\vert b\right\vert }{-2a^{2}\sin(t)^{2}%
\sin(\theta)\cos(\theta)+2b^{2}\sin(t)^{2}\sin(\theta)\cos(\theta
)-2\sin(t)\left\vert a\right\vert \left\vert b\right\vert +4\sin(t)\sin
(\theta)^{2}\left\vert a\right\vert \left\vert b\right\vert }\\
Y=\dfrac{s\left(  -a^{2}\sin(\theta)^{2}\left\vert a\right\vert +a^{2}%
\sin(\theta)^{2}\cos(t)^{2}\left\vert a\right\vert -b^{2}\sin(t)^{2}%
\cos(\theta)^{2}\left\vert a\right\vert -2a^{2}\sin(t)\sin(\theta)\cos
(\theta)\left\vert b\right\vert \right)  }{\left(  -a^{2}\sin(t)\sin
(\theta)\cos(\theta)+b^{2}\sin(t)\sin(\theta)\cos(\theta)+\left(  -\left\vert
a\right\vert +2\sin(\theta)^{2}\left\vert a\right\vert \right)  \left\vert
b\right\vert \right)  \sin(t)}.
\end{array}
\]
...
\right]  .
\]
We display $X_{1}\left(  a,s,t,\theta\right)  $ and $Y_{1}\left(
a,s,t,\theta\right)  $ obtained from Maple \cite{Maple} in the following
screen shots respectively:
\begin{align*}
&
...
%BeginExpansion
{
\includegraphics[natheight=0.790300in,natwidth=6.659500in,height=0.8036in,width=6.5628in]{PJH75I1T.pdf}
}
%EndExpansion
\\
&
...
%BeginExpansion
{
\includegraphics[natheight=0.918900in,natwidth=6.659500in,height=0.9296in,width=6.5628in]{PJH75I1U.pdf}
}
%EndExpansion
\end{align*}
In the meantime, the parametric equation for the locus $J$ can be obtained
from Geometry Expressions \cite{GE}, which can be exported to Maple
\cite{Maple}, which we show below and allows us for further investigation.
\[%
